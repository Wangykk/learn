...
app.on("request", (req, res) => {
  // 读取文件中的内容 响应给浏览器
  let url = req.url; // 获取请求的路径 获取的是端口号后面的所有的内容
  ...
  } else if (method == "GET" && url == "/views/login.html") {
    fs.readFile(path.join(__dirname, "/views/login.html"), (err, data) => {
      if (err) return console.log(err.message);
      res.end(data);
    });
  } else if (method == "GET" && url == "/views/list.html") {
...
// .html .css  .js  .json  .txt  .vue .ejs等等都是文本文件
